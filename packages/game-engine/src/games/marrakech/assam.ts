import type {
  MarrakechAssam,
  MarrakechBorderChoiceInfo,
  MarrakechBorderOption,
  MarrakechDirection,
  MarrakechDiceResult,
  MarrakechPosition,
} from "@alpharena/shared";
import { MARRAKECH_BOARD_SIZE, DIRECTION_VECTORS, OPPOSITE_DIRECTION } from "./constants.js";

function isInBounds(row: number, col: number): boolean {
  return row >= 0 && row < MARRAKECH_BOARD_SIZE && col >= 0 && col < MARRAKECH_BOARD_SIZE;
}

function getBorderTurnOptions(
  pos: MarrakechPosition,
  exitDirection: MarrakechDirection,
): MarrakechBorderOption[] {
  const perpDirs: MarrakechDirection[] =
    exitDirection === "N" || exitDirection === "S" ? ["W", "E"] : ["N", "S"];

  const labels: Record<MarrakechDirection, string> = {
    N: "Up",
    S: "Down",
    E: "Right",
    W: "Left",
...
export function moveAssamUntilBorderOrDone(
  ...
  const movePath: MarrakechPosition[] =
...
export function continueAfterBorderChoice(
  ...
  const newPos: MarrakechPosition = {
